...
async function handleDiscard(uploadId: string) {
                ...
                <div className={`mt-4 pt-4 border-t border-dashed ${video.status === "failed" ? "border-red-600" : "border-rf-outline-variant"} flex justify-between ${video.status === "processing" ? "opacity-50 pointer-events-none" : ""}`}>
                  {video.status === "failed" ? (
                    <>
                      ...
                      <button   onClick={() => handleDiscard(video.uploadId)} className="text-rf-secondary font-[family-name:var(--font-space-grotesk)] text-sm font-semibold tracking-[0.1em] uppercase hover:text-rf-primary flex items-center gap-1 active:translate-y-0.5">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
